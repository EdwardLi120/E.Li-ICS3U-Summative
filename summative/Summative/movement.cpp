...
void playerMovement(ALLEGRO_DISPLAY *display, ALLEGRO_TIMER *timer, ALLEGRO_BITMAP *imageCrosshair, ALLEGRO_BITMAP *imageEnemy, ALLEGRO_EVENT_QUEUE *event_queue, struct playerData * player, struct bulletData * bullet, struct enemyData enemy[][COLS], struct antiMissileData antiMissile) {
	...
	int crosshairHeight = al_get_bitmap_height(imageCrosshair);
	int crosshairWidth = al_get_bitmap_width(imageCrosshair);
	...
	while (!done) {
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {  //update every 1/60 of a second
			...
			if (ev.mouse.x >= 4 && ev.mouse.x <= SCREEN_W - crosshairWidth - 4)
				player->x = ev.mouse.x;
			if(ev.mouse.y >=4 && ev.mouse.y <= SCREEN_H - crosshairHeight - 4)
				player->y = ev.mouse.y;
		}
		...
		if (draw && al_is_event_queue_empty(event_queue)) {
			...
			drawCrosshair(imageCrosshair, *player);
...
void drawCrosshair(ALLEGRO_BITMAP *imageCrosshair, struct playerData player) {
	al_draw_bitmap(imageCrosshair, player.x, player.y, 0);
}
